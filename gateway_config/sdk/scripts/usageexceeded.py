...
verified_pat = r'verified for sufficient SU'
verified_reo = re.compile(ut_pat, flags=re.MULTILINE)
try:
...
for t_tuple in utotal_list:
    ...
    try:
        cp = subprocess.run(["usageLimitsShow", "-u", t_tuple[0]], capture_output=True, text=True, timeout=30)
    except subprocess.TimeoutExpired:
        print('TimeoutExpired for usage.py -t')
        sys.exit(1)
    #if int(t_tuple[4]) > int(xsede_us_su_limit):
    if not verified_reo.match(cp.stdout):
        limit_exceeded = True
        #print(f'user {t_tuple[0]} has exceeded xsede_us_su_limit {xsede_us_su_limit} with total usage of {t_tuple[4]}!\n')
        print(f'user {t_tuple[0]} has exceeded SU Limit!\n')
    else:
        print(f'user {t_tuple[0]} has under SU Limit\n')
